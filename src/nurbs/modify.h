...
namespace nurbs
{
    ...
    namespace internal
    {
            ...
            std::vector<T>& new_knots, array2<glm::vec<dim, T>>& new_cp)
        {
            ...
            // Compute alpha
            array2<T> alpha(degree - s, std::vector<T>(r + 1, T(0)));
            for (int j = 1; j <= r; ++j)
            {
                int L = span - degree + j;
            ...
            else
            {
                ...
                new_cp.resize(cp.size());

                for (int i = 0; i < new_cp.size(); ++i)
                {
                    new_cp[i].resize(cp[0].size() + r);
                }
            ...
            std::vector<T>& right_knots, array2<glm::vec<dim, T>>& right_control_points)
        {
            ...
            if (along_u)
            {
                left_control_points.resize(ks + r);
                for (int i = 0; i < ks + r; ++i)
                {
                    left_control_points[i].resize(tmp_cp[0].size());
                    for (int j = 0; j < tmp_cp[0].size(); ++j)
                    {
                        left_control_points[i][j] = tmp_cp[i][j];
                    }
                }

                right_control_points.resize(tmp_cp.size() - ks - r + 1);
                for (int i = ks + r - 1; i < tmp_cp.size(); ++i)
                {
                    right_control_points[i].resize(tmp_cp[0].size());
                    for (int j = 0; j < tmp_cp[0].size(); ++j)
                    {
                        right_control_points[i - ks - r + 1][j] = tmp_cp[i][j];
                    }
                }
            }
            else
            {
                left_control_points.resize(tmp_cp.size());
                for (int i = 0; i < tmp_cp.size(); ++i)
                {
                    left_control_points[i].resize(ks + r);
                ...
                right_control_points.resize(tmp_cp.size());
                for (int i = 0; i < tmp_cp.size(); ++i)
                {
                    right_control_points[i].resize(tmp_cp[0].size() - ks - r + 1);
                    for (int j = ks + r - 1; j < tmp_cp[0].size(); ++j)
                    {
                        right_control_points[i][j - ks - r + 1] = tmp_cp[i][j];
                    }
                }
            }
        }
    ...
    RationalSurface<T> surfaceKnotInsertU(const RationalSurface<T>& srf, T u, unsigned int repeat = 1)
    {
        ...
        array2<glm::vec<4, T>> Cw(srf.control_points.size(), std::vector<glm::vec<4, T>>(srf.control_points[0].size()));
        for (int i = 0; i < srf.control_points.size(); ++i)
        {
            for (int j = 0; j < srf.control_points[0].size(); ++j)
            {
        ...
        // Convert back to cartesian coordinates
        new_srf.control_points.resize(new_Cw.size());
        new_srf.weights.resize(new_Cw.size());
        for (int i = 0; i < new_Cw.size(); ++i)
        {
            new_srf.control_points[i].resize(new_Cw[0].size());
            new_srf.weights[i].resize(new_Cw[0].size());

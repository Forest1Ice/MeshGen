...
namespace nurbs
{
    ...
    namespace internal
    {
                ...
                unsigned int num_ders, T u, T v)
        {
            ...
            array2<glm::vec<dim, T>> surf_ders(num_ders + 1, std::vector<glm::vec<dim, T>>(num_ders + 1, glm::vec<dim, T>(0.0)));
    ...
    */
    ...
    template <typename T> glm::vec<3, T> surfacePoint(const RationalSurface<T>& srf, T u, T v)
    {
        ...
        // Compute homogenous coordinates of control points
        array2<tvecnp1> Cw;
        Cw.resize(srf.control_points.size());
        for (int i = 0; i < srf.control_points.size(); i++)
        {
            Cw[i].resize(srf.control_points[i].size());
            for (int j = 0; j < srf.control_points[i].size(); j++)
            {
                Cw[i][j] =
    ...
    array2<glm::vec<3, T>> surfaceDerivatives(const RationalSurface<T>& srf, int num_ders, T u, T v)
    {
        ...
        homo_cp.resize(srf.control_points.size());
        for (int i = 0; i < srf.control_points.size(); ++i)
        {
            homo_cp[i].resize(srf.control_points[i].size());
            for (int j = 0; j < srf.control_points[i].size(); ++j)
            {
                homo_cp[i][j] =
        ...
        for (int i = 0; i < homo_ders.size(); ++i)
        {
            ...
            for (int j = 0; j < homo_ders[i].size(); ++j)
            {
                Aders[i][j] = util::truncateHomogenous(homo_ders[i][j]);
            }
        }

        array2<tvecn> surf_ders(num_ders + 1, std::vector<tvecn>(num_ders + 1));
